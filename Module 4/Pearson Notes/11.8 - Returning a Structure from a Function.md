...
Just as functions can e written to return an `int`, `long`, `double`, or any other data type, they can also be designed to return a structure. Recall the following structure declaration from [Program 11-2](/Module%204/Pearson%20Notes/11.3%20-%20Accessing%20Structure%20Members#Program-11-2).
```c++
struct Circle
{
	double radius;
