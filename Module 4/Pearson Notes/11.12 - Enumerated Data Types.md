...
#### Table of Contents
- [11.12 - Enumerated Data Types](#11.12%20-%20Enumerated%20Data%20Types)
- [Assigning an Integer to an `enum` Variable](#Assigning%20an%20Integer%20to%20an%20enum%20Variable)
- [Assigning an Enumerator to an `int` Variable](#Assigning%20an%20Enumerator%20to%20an%20int%20Variable)
- [Comparing Enumerator Values](#Comparing%20Enumerator%20Values)
- [Anonymous Enumerated Types](#Anonymous%20Enumerated%20Types)
- [Using Math Operators to change the Value of an `enum` Variable](#Using%20Math%20Operators%20to%20change%20the%20Value%20of%20an%20enum%20Variable)
- [Using an `enum` Variable to Step through an Array's Elements](#Using%20an%20enum%20Variable%20to%20Step%20through%20an%20Array's%20Elements)
- [Using Enumerators to Output Values](#Using%20Enumerators%20to%20Output%20Values)
- [Specifying Integer Values for Enumerators](#Specifying%20Integer%20Values%20For%20Enumerators)
- [Enumerators Must Be Unique within the Same Scope](#Enumerators%20Must%20Be%20Unique%20within%20the%20Same%20Scope)
- [Declaring the Type and Defining the Variables in One Statement](#Declaring%20the%20Type%20and%20Defining%20the%20Variables%20in%20One%20Statement)
- [Strong Type `enums`](#Strong%20Type%20enums)
...
#### [BACK TO THE TOP](#Table%20of%20Contents)
...
Notice [Program 11-10](#Program%2011-10) does not define a variable of the `Day` data type. Instead, it uses the Day data type’s enumerators in the `for` loops. The counter variable `index` is initialized to `MONDAY` (which is 0), and the loop iterates as long as `index` is less than or equal to `FRIDAY` (which is 4). When you do not need to define variables of an enumerated type, you can actually make the type anonymous. An **anonymous enumerated type** is simply one that does not have a name. For example, in [Program 11-10](#Program%2011-10) we could have declared the enumerated type as:
```c++
...
## Program 11-11
A modified version of [Program 11-10](#Program%2011-10) that is made to use a `Day` variable to step through the elements of the `sales` array.
```c++
   // This program demonstrates an enumerated data type. 
   #include <iostream> 
   #include <iomanip> 
...
In this example, the `FREEZING` enumerator is assigned the integer value 32, and the `BOILING` enumerator is assigned the integer value 212. [Program 11-13](#Program%2011-13) demonstrates how this enumerated type might be used.
